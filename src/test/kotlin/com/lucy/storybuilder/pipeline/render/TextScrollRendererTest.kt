...
class TextScrollRendererTest {
    ...
    private fun renderer(s: RenderSettings): TextScrollRenderer {
        val block = TextLayout().layout("HELLO WORLD", s)
        val timeline = TimelineBuilder().build(listOf(3.0), s.fps)
        return TextScrollRenderer(s, LinearScroll(s.startY, s.scrollSpeed), timeline, listOf(block))
    }
    ...
    fun `buffer is bgr24 with background and text centred on the motion position`() {
        val s = settings()
        val pixels = renderer(s).use { it.render(0).copyOf() }
    ...
    fun `text scrolls up by speed x time`() {
        val s = settings()
        renderer(s).use { renderer ->
            val top0 = inkedRows(renderer.render(0), s).first()
            val top20 = inkedRows(renderer.render(20), s).first() // t = 2 s -> 40 px higher
    ...
    fun `first frame of a scene is fully faded out`() {
        ...
        val pixels = renderer(s).use { it.render(0) }
